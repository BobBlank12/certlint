...
def getFilePem(filename):
    return send_file(getuploadfolder()+filename+"-converted-to.pem", as_attachment=True)
...
def getFileDer(filename):
    return send_file(getuploadfolder()+filename+"-converted-to.der", as_attachment=True)
...
def getFileP12ca(filename):
    return send_file(getuploadfolder()+filename+"-extraxted-ca-certs.pem", as_attachment=True)

@views.route('/success', methods = ['POST'])  
def success():  
    if request.method == 'POST':
        # Cleanup any former posts/files
        oldfiles = glob.glob('website/'+ getuploadfolder() + '*')
        for f in oldfiles:
            os.remove(f)

        f = request.files['file']
        #print ("saving file: website/" + getuploadfolder() + secure_filename(f.filename))
        f.save("website/" + getuploadfolder() + secure_filename(f.filename))
        f.close
        fileformat = "unknown"
        key_type = "n/a"

        # Determine the file format I've been given
        fileformat, key_type = determine_file_format("website/" + getuploadfolder()+f.filename, fileformat, key_type)
        ...
        if fileformat == "pem":
            try:
                convert_pem_to_pem("website/" + getuploadfolder()+f.filename)
        ...
        if fileformat == "der":
            try:
                convert_der_to_pem("website/" + getuploadfolder()+f.filename)
            except Exception as e:
        ...
        if fileformat == "p12":
            try:
                convert_p12_to_pem("website/" + getuploadfolder()+f.filename)
            except Exception as e:
            ...
            try:
                extract_ca_from_p12("website/" + getuploadfolder()+f.filename)
            except Exception as e:
        ...
        if fileformat == "p7b":
            try:
                convert_p7b_to_pem("website/" + getuploadfolder()+f.filename)
            except Exception as e:
            ...
            try:
                found_ca = extract_ca_from_p7b("website/" + getuploadfolder()+f.filename)
            except Exception as e:
        ...
        if fileformat != "key":
            ...
            try:
                convert_pem_to_der("website/" + getuploadfolder()+f.filename)
            except Exception as e:
            ...
            certdetails = get_pem_cert_details("website/" + getuploadfolder() + f.filename+"-converted-to.pem")
